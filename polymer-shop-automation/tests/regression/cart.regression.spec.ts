...
test.describe('Cart @regression', () => {
  ...
  test('TC-R09 cart shows a subtotal row when an item is present', { tag: '@regression' }, async ({ productPage, cartPage }) => {
    const p = products.products[0];
    await productPage.goto(p.category, p.name);
    await productPage.addToCart();
    await cartPage.goto();
    ...
    const text = await cartPage.subtotalText();
    // checkout-box shows "Total: $XX.XX  CHECKOUT"
    expect(text).toMatch(/\$[\d,.]+/);
  });

  test('TC-R10 removing an item decreases the line-item count', { tag: '@regression' }, async ({ productPage, cartPage }) => {
    const p = products.products[0];
    await productPage.goto(p.category, p.name);
    await productPage.addToCart();
    await cartPage.goto();
