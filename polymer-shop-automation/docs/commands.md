...
> Run all commands from the project root: `polymer-shop-automation/`
...
npm run test:smoke       # only @smoke tests  (~30 s — run first to check app is alive)
...
Report files generated after each run:

| Format | Path |
|--------|------|
| HTML (interactive) | `reports/html-report/index.html` |
| JSON | `reports/results.json` |
| JUnit XML (CI/CD) | `reports/results.xml` |
| Markdown | `docs/test-execution-report.md` (via `npm run report:md`) |
...
npx playwright test --grep @sanity               # sanity only
...
HEADLESS=1 npm run test:smoke                                           # CI-style smoke check
```
